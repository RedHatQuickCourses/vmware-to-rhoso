...
. Go back to your bastion node

Download the latest RHEL qcow2 image from access.redhat.com.

[source,bash,role=execute]
----
curl -o rhel9-6.qcow2 "$URL_FROM_ACCESS_REDHAT_COM"
----
...
Copy the RHEL 9.6 (or latest qcow image to the openstackclient pod:
[source,bash,role=execute]
----
oc cp /home/lab-user/rhel9-6.qcow2 openstack/openstackclient:/home/cloud-admin/.
----
...
openstack image create rhel96-image --container-format bare --disk-format qcow2 --public --file rhel9-6.qcow2
----
...
[source,bash,role=execute,subs=attributes]
----
openstack server add floating ip rhel96-ch {rhoso_conversion_host_ip}
----
...
+--------------------------------------+-------------+--------+----------------------------------+----------------+---------+
| b037521d-b583-4ac0-9906-73ad9c0624dd | rhel96-ch | ACTIVE | private=10.0.0.252, {rhoso_conversion_host_ip} | rhel96-image | migrate |
+--------------------------------------+-------------+--------+----------------------------------+----------------+---------+
----
